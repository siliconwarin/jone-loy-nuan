...
function handleSurveyResponse(data) {
  try {
    ...
    const required = ['ageGroup', 'education', 'occupation', 'hasScamExperience', 'socialMediaUsage', 'platforms'];
    for (const field of required) {
      if (!data[field]) {
        throw new Error(`Missing required field: ${field}`);
      }
...
function getQuizStats() {
  ...
  const averageScore = quizData.reduce((sum, row) => sum + (row[4] || 0), 0) / totalQuizzes;
  const averagePercentage = quizData.reduce((sum, row) => {
    const total = row[3] || 7;
...
function getSurveyStats() {
  ...
  const averageScore = surveyData.reduce((sum, row) => sum + (row[4] || 0), 0) / totalSurveys;
